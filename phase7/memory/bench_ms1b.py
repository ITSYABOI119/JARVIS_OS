...
def run_model(model_key, seeds, days, out_path, port, ctx, ngl):
        ...
        for seed in seeds:
            ...
            for s in hh["spans"]:
                ...
                for c in (r.get("candidates") or []):
                    c = dict(c)
                    c.setdefault("speaker_cluster", s["cluster"])
                    preds.append(c)
...
def main(argv=None):
    ...
    res = run_model(a.model, seeds, a.days, a.out, a.port, a.ctx, a.ngl)
    ag = res["aggregate"]
    print()
    ...
    print("relation   : recall %.4f ; preference polarity agreement %.4f"
          % (ag["relation_recall"], ag["preference_polarity_agreement"]))
