...
- **Ranker:** at MS0 `lane_score = 1 / (1 + rank)` within a lane — an ORDINAL score that turns a 7 % BM25 difference into a 2× gap; harmless while every competitor shares source, recency and confidence, and NOT fit for two lanes. **From MS1a the lanes are fused by reciprocal rank: `lane_score = Σ over lanes 1 / (60 + rank)`, rank 1-based within each lane, a row found by several lanes summing its terms; 60 is the constant the method was published with, not a tuned value, and it moves only with a measured reason written here.** The embedding lane embeds the query symmetrically (the same text form as the stored rendering, no instruction prefix — the form C/M0.5 measured better for query-to-query); an instruction-prefixed arm is REPORTED beside it at MS1a, never silently adopted. Then `score = lane_score × w_source × w_recency × confidence`, with `w_source` = 1.0 / 0.8 / 0.6 for `stated_owner` / `stated_other` / `inferred`, and `w_recency` a half-life decay on the age of the newest supporting span — 90 days for events and inferred facts, none for stated profile facts. Ties by `recorded_at`, newest first. The decay is R6's mechanism: it demotes, and a fact with a recomputed high confidence from fresh spans rises again.
