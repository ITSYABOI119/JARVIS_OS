...
| 7.8 | MS0 — the memory store and its rules: schema, the predicate registry, rules R1–R7, audit rows, the purge, the benchmark harness and the synthetic corpus, driven by oracle candidates | `MEASURED 2d78642 2026-09-06 (bands missed: growth_drop<=5)` | 10 seeded households, ORACLE candidates, full-text lane only: update 96.25 %, coexisting 100 %, audit 100 % (0 violations), p99 0.354 ms at 100k facts (p50 0.0985) — four bands met; growth drop 46.25 pts against a ≤ 5 band MISSED, cause measured not argued (30x filler collapses the IDF of `lives in` / `works as` to 0.02, BM25 length-normalisation then returns the shorter fact; the same run with filler on an unasked predicate drops 0.0), nothing retuned; transfer recall@5 0 % and relation precision 1.0 REPORTED not banded; the inferred spouse edge surfaced on day 8 in 10/10 households, dipped to 0.7364 on the day-11 contradiction, finished 0.8647; 114 checks and two CI steps green (run 34032459048) | `phase7/docs/PHASE_7_MEMORY_DESIGN.md` §3, §4, §8, §11 |
