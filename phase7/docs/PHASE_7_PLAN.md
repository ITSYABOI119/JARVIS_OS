...
| 7.8 | V2: speaker-verified commands from the wearable with NO wake word as a gate — always listening; an addressee inference decides that the owner is speaking to JARVIS (its name a cue, never required); the owner's voice verified → control-IN; liveness against a replayed recording and a pre-registered addressee-precision band are this row's to design; delegation to another household voice is an explicit owner act, its own later slice | `NOT STARTED` | — | idea doc §4 |
| 7.8 | V3: the always-on LIVE listener — wireless or live variants so JARVIS hears in real time, 24/7; opt-in sessions for guests | `WANTED (the operator, 2026-09-07: eventually listen 24/7 is the end state; decided last in ORDER, designed after V2)` | — | idea doc §4, §8 |
