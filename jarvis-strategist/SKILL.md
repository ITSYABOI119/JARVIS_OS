...
- QUICK TIER (continuing recent work): the line above + the current-frontier docs is enough context. Skip the agent sweep. (Phases 6–7 have NO weekly cadence — read `phase7/docs/PHASE_7_PLAN.md` §0, the live status board, FIRST, then `phase6/docs/` + the `memory/` Phase-6/7 notes; earlier phases used `phaseN/weeks/`.)
...
- EITHER TIER: if an untracked `HANDOFF*.md` exists at the repo root (`HANDOFF-strag.md` is the strategist's, `HANDOFF-code.md` the coder's — the newer wins on shared facts), read it FIRST — it is the previous session’s state file (untracked by design), and it outranks this file’s snapshot on session-specific state.
...
then get to explore for context say:
  ...
  - Read phase7/docs/PHASE_7_PLAN.md §0 FIRST — the ACTIVE phase's live status board (stood up 2026-09-05, `ae9664c`; a cell reads DONE only with a commit hash + date; nothing on it is done yet; goal 8 = the ambient voice wearable, CANON since 2026-09-05 by the operator's decision; the first arc — 7.1 recall or 7.8's V-pipe — is the OPERATOR'S pending decision, never pick it)
...
- Check **`phase7/docs/PHASE_7_PLAN.md` FIRST — the ACTIVE phase (stood up 2026-09-05); its §0 is the LIVE STATUS BOARD (a cell reads DONE only with a commit hash + date, written in the same commit that lands the work; the strategist's prompt names the row to flip; the carried hardening backlog sits under the board)** + `phase4/docs/ROADMAP.md` (Phases 4-7 + cross-phase backlog B1/B2/B3) + **`phase6/docs/` — the CLOSED-phase record set: `PHASE_6_PLAN.md`, the `PHASE_6_GOAL_K_*` docs (keystone, done), `PHASE_6_GOAL_6-1_MONITORS.md`/`6-2_EVENT_WAKE.md`/`6-3_PROACTIVE.md` (done + flipped default-ON), `PHASE_6_GOAL_6-5_CONTROL_IN.md` + `PHASE_6_GOAL_6-6_ROUTING.md` (both done + flipped default-ON), `PHASE_6_GOAL_6-7_SOAK.md` (the runbook for the DEFERRED, owner-scheduled supervised run — Phase 6 CLOSED 2026-09-05 by ADR; never propose its timing), `SOAK_2026-08_FINAL_REPORT.md` (the 2026-08 unattended soak verdict), `PHASE_6_GOAL_C_EMBEDDER.md` + `PHASE_6_GOAL_C_M1B_DESIGN.md` (Phase C — SHIPPED through C/M4, now dormant), `ANSWER_QUALITY_DESIGN.md` + `THINKING_MODE_RESEARCH.md` + `MODEL_BENCH_2026-07.md` + `MODEL_OPTIONS_2026-07.md`** + the `memory/` Phase-6 notes; `phase5/docs/`, `phase4/docs/PHASE_4_FINAL_REPORT.md`, and `phase3/docs/PHASE_3_FINAL_REPORT.md` are now the closed-phase records (Phase 5 is COMPLETE)
- Check `docs/decisions/` for ADRs — they override stale plan-doc rows (dynamic scaling removed 2026-04-17; 30-day soak + TurboQuant/RotorQuant deferred 2026-06-15; GPU inference deferred + x86 verification stance 2026-06-16; SMP Branch A 2026-06-17; headless appliance 2026-06-21; target-disk install [Proposed] 2026-06-25; Phase 6 closed + the supervised 6-7 run deferred 2026-09-05)
...
7. **A relayed prompt is IMMUTABLE.** The moment you tell the user to send it, stop editing it. A new finding goes in a NEW `PROMPT-<TOPIC>.md`, never into one already in flight. **And "unrelayed" is a belief that expires every time the user speaks: before ANY edit to a prompt you think is still waiting, `git fetch && git log --oneline -3` and `ls REPORT-*.md` — if a commit matching it has landed, the prompt is consumed; delete it and put the change in a NEW `-AMEND` prompt against the LANDED text's anchors.** (2026-09-07: `PROMPT-VOICE-COMMAND-RULE-V2.md` landed as `9978699` while three clarifying answers were being edited into it; the answers never landed and needed an amendment.)
...
### Phase Status
Snapshot as of 2026-09-05 — CLAUDE.md + `phase7/docs/PHASE_7_PLAN.md` §0 + `phase6/docs/` + the `memory/` Phase-6/7 notes are the truth; verify before relying on a row, and treat any mismatch as SKILL.md drift to fix (see the self-check in the exploration prompt).
...
| Phase 6 | **CLOSED 2026-09-05 — Butler. Keystone K ✅ + 6-1 monitors ✅ + 6-2 wake ✅ + 6-3 proactive ✅ + 6-5 control-IN ✅, ALL flipped default-ON.** The deployed image self-heals BOTH **crashes** (fault-EP respawn) AND **wedges** (`km2b_miss`) — SHIELD-scored + JACT-audited — runs always-on monitors + event-driven wake + ≥5 INFORM behaviors, and holds a **two-way authenticated conversation with cross-session recall** (`JARVIS_CONTROL_IN` default-ON `a9c1d9a` 2026-07-21 + `JARVIS_CONTROL_IN_RECALL` default-ON `1fd505d` 2026-07-22 — closes 6-5's last §14 done-when at the exact-repeat level, on a dedicated control-IN episodic store @ LBA 21,140,000). **`JARVIS_ACTIONS` default-ON `34a165e` (2026-07-08); SEC-039 CLOSED for the ACTION path AND for control-IN queries** (`query_shield.c` scores every inbound query, refusal audited by reason-class LABEL only); the deployed WORKLOAD PA↔PB query path stays passive/ALLOW — NOT a query blocker. **6-6 routing ✅ default-ON `d4be861` 2026-07-23 (HELDOUT 70/73 = 95.89% keyword-blind, 0 INFER misroutes, boot_id=38). 6-7 CLOSED by ADR `docs/decisions/2026-09-05-close-phase-6-defer-supervised-exit.md`: its done-when (a 7-day run whose JACT audit trail shows no Level 2+ action taken without approval) is MET IN SUBSTANCE by the 2026-08 unattended soak (boot 54: 7 d 18 h 18 m, err=0, a 42-record JACT trail all AUTO/NOTIFY, 0/3 unscheduled informs, ended by grid power), and its SUPERVISED clause is DEFERRED, owner-scheduled — never propose its timing. Final report `phase6/docs/PHASE_6_FINAL_REPORT.md`.** Docs: `phase6/docs/`. |
| Phase 7 | **ACTIVE — stood up 2026-09-05 (`ae9664c`) — Autonomy.** The plan `phase7/docs/PHASE_7_PLAN.md` carries the LIVE STATUS BOARD as §0 (one row per goal-arc-milestone; a cell reads DONE only with the commit hash + date, written in the SAME commit that lands the work; the strategist's prompt names the row to flip). Nothing on the board is done. Canon goals 7.1–7.7 quoted verbatim from ROADMAP; the first arc is the OPERATOR'S CHOICE — 7.1 paraphrase recall (baseline 19/36 = 53 % at the 0.55 floor, done-when ≥80 %; off-box measurement first, a 2070 fine-tune only on a measured miss) vs 7.8's V-pipe (Main-PC only). **Goal 8 = the ambient voice wearable, CANON since 2026-09-05 by the operator's decision — goals 1–7 unchanged; the owner's decisions of 2026-09-06 shape it: ONLY THE OWNER ENROLLS (from the Main PC headset mic; master his voice FIRST); JARVIS must GUESS, over days and untold, that the recurring second voice is his wife and who she is to him — a deliberate challenge; EVERYTHING is transcribed, raw audio deleted after transcription, non-household transcripts purged by the owner BY HAND (the idea doc's §3 discard-before-storage rule SUPERSEDED — the owner's accepted legal risk, recorded not hidden); inferred facts carry a confidence and are USED without owner confirmation; a purpose-built household-profile store on the Main PC that is to be STATE OF THE ART — the owner's priority ("I don't care if we spend a whole phase working on the one thing"), researched before it is designed, the same substrate 7.1 (associative memory) and 7.5 (personality) need; a CLEAN console view (Claude Design) + a digest; continuous local recording IS the v1 wearable (radios off, USB-C batch upload — streaming stays off); stages owner-voice → transcribe-all pipeline → profile store + the guess → console view → digest → V0 → V1 → V2 are board rows, V3 (wireless / guest sessions) BLOCKED, decided last.** 7.2 (30-day run) OPERATOR-SCHEDULED — never propose timing; 7.4 BLOCKED on hardware (ADR 2026-06-16); 7.6 external; 7.7 last. The carried hardening backlog (the undeployed `3f676a2`, the dead-linked ring modules, `LOG_ERROR`, the `head_dim` stack arrays at `llama_quant.c:1433/1442/1532` — recorded, not fixed — the coverage backlog, B3's quickstart, the two open CI decisions) sits under the board. |
...
Current milestone: do NOT hardcode here (it moves) — Phases 6–7 have NO weekly cadence; read `phase7/docs/PHASE_7_PLAN.md` §0 (the status board — DONE only with a hash) + `phase6/docs/` + the `memory/` Phase-6/7 notes + the last ~15 commits (commits win on any disagreement).
...
- [ ] **Every pre-registered grep gate has been run against the prompt's OWN dictated text before relay** — a positive control on the unedited file (which lines the edits are meant to remove) AND the gate applied to the replacement text the prompt itself dictates. Twice in one night (2026-09-05/06) a gate disagreed with the dictated text and the coder had to stop: a bare `candidate` gate against dictated "candidates for the first arc" (plus the plan's ordinary "candidate lever"), then a case-sensitive `SUPERSEDED` count of ≥ 2 when the dictated W1 said "SUPERSEDES". A gate that cannot pass on the text you wrote is a self-contradiction, and the anti-corruption rule makes the coder stop on it — correctly, at the cost of a round-trip each time.
...
3. Checking `phase4/docs/ROADMAP.md` (incl. the B1/B2/B3 backlog + the Beyond-Phase-7 table — the ambient-voice-wearable idea lives there) + **`phase6/docs/`** (all goal docs — derive the frontier fresh; as of 2026-09-05 Phase 7 is the ACTIVE phase — read `phase7/docs/PHASE_7_PLAN.md` §0, its live status board, FIRST (nothing on it is done; goal 8 = the bracelet, canon since 2026-09-05; the first arc — 7.1 or 7.8's V-pipe — is the operator's pending decision); the box is parked on Ubuntu, the pending-deploy delta is ONE undeployed commit (`3f676a2`, the 2026-09-04 -Wall cleanup of `main_x86.c`, KVM-gated), Phase 6 is CLOSED (2026-09-05, ADR; the supervised 6-7 run deferred and owner-scheduled — `PHASE_6_GOAL_6-7_SOAK.md` stays its runbook), the Phase 7 home LANDED the same day (`ae9664c`); remaining hardening work sits in the plan's carried-backlog table — see the Current-state paragraph in §2) + the `memory/` Phase-6/7 notes + `docs/decisions/` ADRs + any untracked `HANDOFF*.md` (`HANDOFF-strag.md` the strategist's, `HANDOFF-code.md` the coder's; the phase3/phase4/phase5 plans are historical — Phase 5 COMPLETE)
