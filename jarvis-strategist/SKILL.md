...
then get to explore for context say:
  ...
  - Read .github/workflows/ci.yml (THREE jobs: `test` = the gating suite, 136 named steps as of 2026-09-01 — count fresh with a duplicate-key-aware parse, `yaml.safe_load` silently keeps the last duplicate key and cannot validate this file; plus `coverage` and `boot-smoke`, both workflow_dispatch-only and non-gating)
...
**Current state (2026-09-01): the box is PARKED on Ubuntu post-outage, nothing armed, pending-deploy delta vs the ESP = ZERO, and there is NO active build arc.** The operator deferred the next soak ~a month (≈ October) — **do not propose soak timing; the operator schedules runs.** Open work is opportunistic, none urgent: receiver durable reply-drop logging (2 of 39 soak replies were lost on the return path, unattributable box-side) · the `[23:00110]` recall-provenance KVM probe (provenance recorded a src whose stored answer contradicts the reply — under-reporting vs model override, unverified) · the coverage worth-fuzzing backlog · pre-soak checklist items (Pi persistent journald; archive the SOLE-COPY soak pcap ring at Pi `/home/pi/soak/` before any capture restart; record an episodic-store lifetime baseline) · the `main_x86.c:4504` stale comment (durable entries are 3 per 100q since `9c772f8`, not 2 — rides the NEXT deployed-path commit, never a docs pass).
