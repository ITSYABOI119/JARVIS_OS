...
- **NIC Intel I211:** `phase3/src/drivers/nic_i211.c/h` — **LIVE (goal #2b N-a)**: TX-only first-light on bare metal. `i211_nic_init` (reset settle + STATUS.PF_RST_DONE + MAC-validity via RAH.AV + TXDCTL.ENABLE), `i211_send_phys` (writes the TX buffer's PHYSICAL addr — IOMMU off). Rootserver brings it up NON-FATAL (BAR0 32pp UC + 2 DMA frames, one broadcast frame, DD-polled) in the `main_x86.c` `[NET]` block. Host `test_nic_i211.c` 11/11 (B1 phys-addr + B2 TXDCTL).
...
- **Phase 3:** 38,082 LOC, 129 code files, 42 test files (IN PROGRESS — **11/11 models, 6 families, qdot + seL4 threadpool M3; +framebuffer font/panel/event-log + jarvis_ui_tokens.h + test_framebuffer.c**)
- **Total (phases):** 111,074 LOC, 300 code files, 90 test files (648 tracked files repo-wide)
