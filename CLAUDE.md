...
| G3 retrieval scorer+assembler (Phase 5 G3/M0) | — | DONE | g3_retrieval.c/h, test_g3_retrieval.c | host CI: 19 PASS (exact-key+recency select, byte-exact preamble, truncation 0xAA-canary, *_len NUL-safety); M1: PA scores+packs into the pool (gated JARVIS_G3_RETRIEVAL, default off) — LANE A host-tested; **M1 BOX-VERIFIED 2026-06-28** (`94070f0` links g3_retrieval.c into Process A; QEMU/KVM smoke — `[RETR]` packs preambles hit=0 recency + hit=1 exact-key facts=2 len<512, err=0, no faults, generation byte-identical OFF↔ON) |
