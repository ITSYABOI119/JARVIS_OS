...
- **Episodic Store (Phase 5 G1/M0):** `phase3/src/ai/episodic_store.c/h` — raw-LBA circular 512 B-record store (nvme_log pattern, struct+callback-driven so the later semantic/SHIELD stores reuse the core; no device dependency — M1 wires the NVMe callbacks). Record = `{boot_id, seq, t_ms, query_key, action, outcome, feedback, query, resp}`; `query_key = cache_hash(cache_normalize_query(query))` (decision-cache FNV-1a parity for #6 cache-growth). Host-tested `test_episodic_store.c` (6/6: fresh-init, append+read-back round-trip, boot_id increment, checksum corruption, wrap+migration, key-parity) → CI step "Phase 5: Episodic Store (C)".
