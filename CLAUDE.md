...
| **Total** | | | **132 code files (45 test)** | **459+ test asserts, ~35,900 LOC** |
...
| Boot time | <60s | ~2s |
| SHIELD block rate | >90% target | **Host harness only: 100% harmful blocked / 0% FP (test_shield.c on shield.c). LIVE query PA↔PB path is passive/monitoring — Process B returns ALLOW (SEC-039). Phase 6 K/M1 (2026-07-04): the SEC-039 closure MECHANISM is proven on the box — the linked ACTION gate refuses a blocklisted action live (induced-BLOCK + audited) — but gated `JARVIS_ACTIONS` default-0, so SEC-039 is NOT yet closed in deploy; it fully closes at the K/M4 flip.** |
...
- **Debug Config:** `phase3/src/sel4/jarvis_debug.h`
  ...
  - `JARVIS_ACTIONS` — **Phase 6 K: the it-acts spine (default: OFF).** When 1, Process A inits the JACT action-audit store (@ LBA 21,120,000) and links the SHIELD action gate (`shield_action.c` — the SEC-039 closure MECHANISM). Gated → deploy byte-identical + **ACTION-INERT** (no action executes at M1 regardless — execution is K/M2; the deploy default-ON flip is the deliberate K/M4 decision).
  ...
  - Committed defaults ARE the stability config: STATS + INFER_SUMMARY + **G3_RETRIEVAL** (default-ON since G3/M6, 2026-07-02) + **CACHE_GROWTH** (default-ON since #6/M3, 2026-07-03) on, IPC/PB/RING/BOOT_LOG/G3_PROBE/G3_AB/SHIELD_LEARN/SHIELD_PROBE/**ACTIONS/ACTION_PROBE/KM2A_SPIKE** off (BOOT_LOG off avoids NVMe write wear over 30 days; SHIELD_LEARN is #5's monitor-only risk map — flip is an M4 call; **ACTIONS/ACTION_PROBE are Phase 6 K — the deploy image stays action-inert until the K/M4 `JARVIS_ACTIONS` flip**)
...
- **Security:** March 2026 adversarial audit — 26 findings, all resolved (phase2+phase3 C). April 2026 Phase 3c audit — 25 findings (all 5 HIGH + 5 MEDIUM fixed, 7 LOW/INFO accepted). Phase 2's own audit: 6 C findings (4 fixed) + Snyk. SHIELD: host harness only (test_shield.c exercises shield.c — keyword + model-assisted scoring); the LIVE query PA↔PB path is passive — Process B returns ALLOW, shield.c not linked on the query path (SEC-039), so the deployed system is NOT a live blocker. **Phase 6 K/M1 (2026-07-04) proves the SEC-039 closure MECHANISM on a SEPARATE ACTION path** — `shield_action.c` linked into Process A refuses a blocklisted action live on the box (induced-BLOCK, audited to JACT) + the K-e learned-risk loop (75→85 BLOCKED on the 2nd attempt) — but gated `JARVIS_ACTIONS` default-0, so this is "closure mechanism proven, deployed-live pending the K/M4 flip", NOT "SEC-039 closed" or "live SHIELD deployed".
