...
| Phase 6 | IN PROGRESS | Months 36+ | Butler — plan (`phase6/docs/PHASE_6_PLAN.md`) + keystone doc (`PHASE_6_GOAL_K_IT_ACTS.md`, K = "it-acts" spine + live SHIELD, closes SEC-039). **K/M0 LANDED 2026-07-04 (host decision core: allowlist + linked-SHIELD gate + JACT action-audit store, host-tested 4+6+4+46; NOTHING links into the deployed path — box wiring is K/M1, gated)** |
...
- **Debug Config:** `phase3/src/sel4/jarvis_debug.h`
  ...
  - Committed defaults ARE the stability config: STATS + INFER_SUMMARY + **G3_RETRIEVAL** (default-ON since G3/M6, 2026-07-02) + **CACHE_GROWTH** (default-ON since #6/M3, 2026-07-03) on, IPC/PB/RING/BOOT_LOG/G3_PROBE/G3_AB/SHIELD_LEARN/SHIELD_PROBE off (BOOT_LOG off avoids NVMe write wear over 30 days; SHIELD_LEARN is #5's monitor-only risk map — flip is an M4 call)
