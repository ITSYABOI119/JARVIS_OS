...
- **Telemetry Console:** `phase4/console/` — honest, read-only browser console for the Remote Telemetry Console (5 screens: CommandCenter / Routing / LastResponse / Models / SHIELD). Reuses the JARVIS design system; served by `telemetry_receiver.py --web-dir phase4/console`; consumes the `/events` SSE (`EventSource`) with recv_ts liveness + stale watchdog, CRC gate, seq-gap drop count, cold-start guards, and one real rolling-buffer sparkline; box-free preview is clearly badged **SIMULATED**. No GPU / live-tok-s / model-tiers / SHIELD-blocking / "verified" — CI-enforced by `phase4/console/test_console_honesty.py` (honesty grep over the authored source, 34/34). goal #2b N-c-3b/c. (`_ds_bundle.js`/`styles.css`/`tokens/` are the vendored DS runtime — excluded from the honesty scan, as they carry the other kits' demo data.)
