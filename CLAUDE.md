...
- **Telemetry fixture (shared packer):** `phase3/scripts/telemetry_fixture.py` — `build_packet`/`build_pcap_many`/`frame_to_packet` + `REQUIRED_RECORD_KEYS` (derived from `packet_to_record`, never hardcoded). Used by `test_telemetry_receiver.py` (key-contract Layer A → 106/106) and `gen_golden_pcap.py`.
- **Golden pcap generator:** `phase3/scripts/gen_golden_pcap.py` — regenerates `golden.pcap` from the JSON (guards `struct.calcsize==222` (v5) + the canonical 0xCBF43926 CRC vector); CI step "Phase 4: Golden telemetry fixture up-to-date" fails on drift.
