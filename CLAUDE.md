...
- **Telemetry Packet:** `phase3/src/drivers/jarvis_telemetry.c/h` — 208-byte (v2) packed versioned `telemetry_packet_t` + zlib/IEEE CRC-32 (`jarvis_tlm_crc32`/`jarvis_tlm_finalize`); the `main_x86.c` workload loop fills it from live state and emits it over the I211 (`net_udp` + `i211_send_phys`, fire-and-forget) at the `[STATS]` cadence + a 1 Hz keepalive (also ticked from the inference response-poll → true ~1 Hz). **System fields (Tier 0/1):** `total_ram_mb` is now real (sum of non-device untypeds) and 4 fields ride former reserved bytes — `infer_active`, `infer_duty_pct` (workload duty cycle = inference cycles/uptime, NOT a CPU gauge — PA busy-polls), `nvme_total_mb`, `log_cursor` (telemetry-log fullness, of 2700); v1 was 200 B (CRC@196; `episodic_count` repurposed from reserved2 at P5 G1/M4); **v2 (P5 G2/M4)** appends `pool_events`/`pool_decisions` before crc32 → **208 B, CRC@204, version 2**, +`TLM_F_CONTEXT` 0x40. goal #2b N-c-1 + P5 G1/G2 memory metrics. Host `test_jarvis_telemetry.c` 34/34.
