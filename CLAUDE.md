...
| Cache-growth promotion selector (Phase 5 #6/M0) | — | DONE | cache_growth.c/h, test_cache_growth.c | host CI: 10 PASS (threshold, dedup, newest-wins, n=0, max-cap) |
...
- **Cache Growth (Phase 5 #6/M0):** `phase3/src/ai/cache_growth.c/h` — pure-C promotion **selector** (`cg_select_promotions`: counts `query_key` frequency over a scanned episodic window, promotes keys with frequency ≥ `CG_PROMOTE_THRESHOLD`=2, emits the **newest** candidate per key, dedups by key; borrowed text carried by pointer + `*_len`, never `strlen`'d — no seL4/device/`decision_cache` dep). Frequency-based + deterministic — the cache **learns frequently-asked queries and serves them fast**, never "understands". Host-tested `test_cache_growth.c` (**10/10**: threshold, dedup, newest-wins, n=0, max-cap) → CI step "Phase 5: Cache-Growth promotion logic (C)". **The dormant SEC-024 LRU eviction goes LIVE + tested** (`decision_cache.c:214-238`, the phase3 copy's fill-then-evict path, unreached at ~308/512): `test_decision_cache_lru.c` (**10/10**: fill to `CACHE_SIZE`=512 → access-control the LRU → insert a 513th key → oldest-accessed evicted, `entries_used` stays 512, new key present, recently-accessed survivors kept) → CI step "Phase 5: Decision-cache LRU eviction (C)". M0 = pure logic + LRU host-test (host/CI); the bounded promote pass into Process A is M1 (box). Design: `phase5/docs/PHASE_5_GOAL6_CACHE_GROWTH.md`.
