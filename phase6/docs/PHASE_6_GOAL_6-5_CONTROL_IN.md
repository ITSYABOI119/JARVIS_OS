...
**Status of these bullets at the 2026-07-21 flip:**
...
| 5 | OFF object-identical; cannot receive until the flip; flip reversible | ✅ MET |
| 6 | The 6-3 trust seam wired, lane latent on the box | ✅ MET as written (the bullet itself scopes the lane as latent) |
...
2. **`main_x86.c:2164` is now stale:** it passes `control_in_available = false` to `trust_policy()`
   ...
   first `TRUST_REQUEST` action lands, and would silently downgrade an approval request to
   `ACT_PROPOSE_LOG`. Fix it with, or before, that action.
