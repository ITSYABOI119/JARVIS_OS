...
- **K/M2b — implement the chosen respawn** behind `JARVIS_ACTIONS` (default 0): promote `pb_restart_entry` to a real respawn wired to detection (§5, fault-EP + heartbeat-age) + the ordered sequence (§6) + audit (§8) + the two probes (§9); the parked-worker reuse becomes a multi-worker restart (a genuinely-faulted worker gets the §4.2-B launch-triple); §3 hoist promotes the spike's pointer-into-frame stash to true file-scope statics (§4.2 G). Flag-OFF = deployed path byte-identical.
