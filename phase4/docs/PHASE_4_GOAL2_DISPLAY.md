...
2. **Optional NVMe progress bar** on top of the exact data-only `%`.
...
4. **LOW-pri hardening: reject `bytes_per_sector != 512` in `fat32_init`.** The FAT-sector cache (and the prior stack buffer) is 512-byte-only; a malicious/corrupt BPB declaring 1024/2048/4096 would read a full sector into the 512-byte buffer. Pre-existing (the old stack `sector_buf[512]` had the identical shape), SEC-029-guarded at the entry-offset level, and never fires on the box's 512B-sector NVMe — but worth an explicit reject + SEC tag + test.
