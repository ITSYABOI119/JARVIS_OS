...
- **CI generation smoke** — **PARTIAL 2026-09-05:** the HOST engine now generates in CI on every push — the `model-tests` job runs `test_generation` and its three sibling suites on a hash-pinned Llama 3.2 1B Q4_K_M, the suites that had only ever SKIPPED with `::warning::`. Still open, and the larger half: the SHIPPED seL4 image never generates in CI (`boot-smoke` boots it with an 8 MiB stub under `JARVIS_MODEL_FAIL_PROBE=1`). The same tiny-model QEMU boot becomes a CI step that asserts end-to-end generation (boot → NVMe load → PA↔PB IPC → coherent tokens), closing the largest untested-in-CI surface.
