...
**Status:** CLOSED 2026-09-05 (ADR `docs/decisions/2026-09-05-close-phase-6-defer-supervised-exit.md`; final report `phase6/docs/PHASE_6_FINAL_REPORT.md`) — the keystone **K ("it-acts", self-heal + the SHIELD action gate)** plus goals **#1 always-on monitors**, **#2 event-driven wake**, **#3 proactive actions** and **#5 natural language primary (control-IN)** are **COMPLETE and DEPLOYED default-ON**. **Goal #5 was the LAST hard-security goal of the phase** — it opened the box to its first untrusted network inbound and had to clear a 6-item security checklist before it could ship. **Goal #6 (routing) is now COMPLETE + DEPLOYED default-ON (2026-07-23). Goal #7's done-when is MET IN SUBSTANCE by the 2026-08 unattended soak (7.76 d, err=0, a 42-record JACT trail all AUTO/NOTIFY); its SUPERVISED clause was not exercised as designed and is DEFERRED, owner-scheduled.** Goal #4 (user model) rides Phase 5's semantic store, which is mechanism-proven and activates with real interaction now that control-IN is live.
...
7. **7-day supervised autonomy** — JARVIS runs 7 days with you present: proactive actions logged, zero unapproved high-risk actions, <5% false-positive interrupts. — **✅ MET IN SUBSTANCE 2026-09-05 (ADR): the 2026-08 unattended soak ran 7 d 18 h 18 m at err=0 with a 42-record JACT trail (3 digests + 39 control-IN turns, nothing above trust level 1, nothing to approve) and 0 unscheduled informs of 3; the "with you present" clause was exercised only for two operator sessions (39 turns incl. a 12-turn adversarial battery) — accepted residual; the supervised run stays owner-scheduled from `phase6/docs/PHASE_6_GOAL_6-7_SOAK.md`.**
...
- [x] 7-day test completed with SHIELD audit trail showing no Level 2+ actions taken without approval — *met in substance 2026-09-05: the 2026-08 unattended soak (7.76 d, err=0) carries a 42-record JACT trail in which no Level 2+ action exists to approve (structural — the deployed allowlist is L0–1 only — and confirmed by the trail); the supervised clause was not exercised as designed and is deferred, owner-scheduled (ADR 2026-09-05).*
...
- **CI generation smoke** — **BUILT 2026-09-05, manual + non-gating:** the shipped seL4 image now boots in CI against a REAL hash-pinned GGUF (Llama 3.2 1B Q4_K_M as `GEMMA2B.GUF`, deploy config untouched), reads it off the emulated NVMe, maps it, and generates through PA↔PB — the `generation` leg of `boot-smoke`. Asserted: the read, the map, the deploy-signature prefix, ≥5 `[INFER]` summaries, zero timeouts/faults/FATAL. Reported, not asserted: `err` and the texts (Process B's template ids are Gemma-only, so a Llama vocabulary yields junk-context generations — plumbing, not quality). The HOST engine has generated on every push since the same day (`model-tests`). Still open: promotion of the image leg to per-push gating once it has a stability distribution, and the 5-minute QEMU quickstart.
