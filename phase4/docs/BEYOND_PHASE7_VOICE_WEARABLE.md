...
- **Commands: the owner's verified voice only, and no wake word as a gate (2026-09-07).** Listening and learning cover everyone in earshot, and the end state is an always-on LIVE listener, 24/7 — the owner's words: "it will not just be wake word thing, i want it to eventually listen 24/7". JARVIS infers when it is being addressed, the way a person in the room would; its name is a natural cue, never a requirement. Acting covers the enrolled owner only, after speaker verification; a command in any other voice is declined and logged, never executed; "unless I say so" — delegation to another household voice — is an explicit owner act and a later slice with its own design. §4's V2 row (a wake-word gate on the device) is SUPERSEDED by this bullet, and its V3 row moves from "only if still wanted" to WANTED, last in order; §4 stays as the 2026-09-01 record. Verification establishes who spoke, not that it was live: a replayed recording of the owner would pass it, so the bound on harm stays structural — the signed control-IN channel and the static action allowlist (K-b) — and liveness plus a pre-registered addressee-precision band are V2's to design and measure. Real-time acting needs live audio, which the V1 batch recorder (radios off, nightly upload) cannot provide; that is what V3 is for. The owner's own path to it (2026-09-07): the bracelet records all day and uploads, JARVIS learns, the bracelet is updated, and the loop continues until the bracelet itself listens and learns in real time. When JARVIS is unsure whether it was addressed it stays silent and logs the utterance, and the console shows what it nearly acted on. JARVIS answers in text for now (the console and the channel); speaking back is not designed yet.
