...
### Q1 — Is AVX2 safe in a Ring-3 seL4 process? **Was NO (FXSAVE kernel); now ✅ YES after M0** — kernel rebuilt to XSAVE/feature-set 7; gate passed (XCR0.avx=1 in PA+PB, 0 mismatches over 24,064 probes). See the M0 RESULT in §2. The analysis below is why it was unsafe and the fix M0 applied.
...
- **Forfeits formal verification (and the guarantee is real on x64 today):** the current **uniprocessor x64** build IS a verified seL4 configuration — the verified-configurations page lists `X64`/`PC99` with *"C-level functional correctness"* (<https://docs.sel4.systems/projects/sel4/verified-configurations.html>). Going SMP drops to the multicore (BKL) kernel, which is **not yet verified** ("verification in progress for static multikernel configs", FAQ; SMP listed as a "mature unverified configuration", <https://sel4.systems/Verification/proofs.html>). So this is a genuine forfeit, not a theoretical one — the headline trade-off, to be recorded in an ADR if pursued.
...
### M0 — Kernel XSAVE/AVX enablement + AVX2 safety spike  *(JARVIS-PC; kernel rebuild)* — ✅ **DONE (2026-06-16, gate PASSED)**
The config read proves AVX is **not** saved today, so M0 is **not** a fast confirm — it requires a kernel rebuild first.

> **RESULT (KVM `-cpu host`, ~6 min under load):** kernel rebuilt `KernelFPU=XSAVE` / feature-set 7 / size 832 / XSAVEOPT (requires **`SIMULATION=OFF`** — the seL4 cmake-tool force-sets FXSAVE under simulation; fixed + two-pass feature-set, now in `build_jarvis_x86.sh`). Boots clean, **self-test 5/5 PASS** (scalar engine unaffected). The `target("avx2,fma")`-isolated probe (`avx2_probe.h`, `JARVIS_AVX2_PROBE` guard, default 0) confirms **XCR0=0x7 (avx=1) in both Process A and Process B** and **0 mismatches over 24,064 PB YMM-reduction probes** (each spanning preemptions, interleaved with PA's YMM touch) + **0 faults**. ⇒ AVX2 in Ring 3 is safe. Model-gen regression deferred to M1 on bare metal (2.96 GB load over emulated NVMe too slow in QEMU; FPU mode is orthogonal to model loading). Detail: `phase4/weeks/week02/WEEK_02_STATUS.md`.
...
- **Exit criteria:** correct generation unchanged; **tok/s scales with workers — target ~4–6 tok/s Gemma 4 E2B** (dual-channel DDR4 **bandwidth knee** at **N≈5–6 — swept, not 8**; batch-1 decode streams ~2.9 GB weights/token, so the memory bus caps useful N, not the 8-core count). Production `NUM_NODES` chosen here after the N-sweep (M2 spike used 2).
...
- **CORRECTED (M0, 2026-06-16) — JARVIS's specific build is NOT a verified X64 configuration; the verification guarantee is already spent by the fast path.** The x86-64 *architecture* has a verified config, but its verified properties are *"C-level functional correctness, **no fast path**"* (<https://docs.sel4.systems/projects/sel4/verified-configurations.html>; uniquely X64 — the AArch32/AArch64 verified configs say *"including fast path"*), and the box runs **`KernelFastpath:BOOL=ON`**. So the formal-verification guarantee is **NOT in force on this build today** — independent of XSAVE/AVX and of SMP. This *corrects* the earlier "uniprocessor x64 runs a genuinely verified config" (true for the *architecture*, false for *this build* due to the fast path). The verified config (`X64_verified.cmake`) also inherits the **FXSAVE** default, so M0's `KernelFPU=XSAVE` is a *second, independent* deviation — but not the load-bearing one.
- **Re-priced M2 verification objection:** because the fast path already spent the C-level guarantee, going SMP does **not** "forfeit a guarantee we currently hold." The M2 Branch-A trade-off should be argued on **maturity** (the multicore BKL kernel is less battle-tested; SMP verification is only *"in progress"*, FAQ <https://sel4.systems/About/FAQ.html>), **not** on losing a proof this build lacks. Regaining a verified X64 kernel would require `KernelFastpath=OFF` + FXSAVE + uniprocessor — sacrificing both IPC and AVX perf, off the table for a perf phase. (CLAUDE.md's "formally verified" claim now carries a qualifier; more mentions to reconcile project-wide — follow-up.)
