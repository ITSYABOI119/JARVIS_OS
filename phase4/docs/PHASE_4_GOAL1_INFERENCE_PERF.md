...
- **Branch A — enable SMP spike:** rebuild `-DSMP=ON -DNUM_NODES=4` (up to 8); confirm boot + `bootinfo->numNodes==N` + PA/PB + NVMe model load still work; a `seL4_TCB_SetAffinity` smoke test proves a thread runs off core 0 (per-core counter). **Accept the verification-guarantee forfeit (write an ADR).** Note: this forfeit is *real, not theoretical* — the current **uniprocessor x64** build IS a verified seL4 configuration (`X64`/`PC99`, C-level functional correctness, <https://docs.sel4.systems/projects/sel4/verified-configurations.html>); the multicore/BKL kernel is not yet verified. The trade-off stands as written.
...
- **RESOLVED — x86-64 IS in seL4's maintained verified set (functional correctness to C, uniprocessor only).** Contrary to the initial assumption, the x64 port is *not* merely "supported but unverified": seL4's verified-configurations page lists `X64`/`PC99` as a verified configuration. *"Configurations that start with `X64` are verified configurations for the x86-64 architecture … The following properties are verified: C-level functional correctness, no fast path … platforms supported by verification: PC99."* (<https://docs.sel4.systems/projects/sel4/verified-configurations.html>). x64 verification is **weaker** than 32-bit Armv7 (which adds binary/translation validation + security proofs) and **excludes** VT-x and the fast path; the FAQ confirms *"For 64-bit Arm v8 and x86 there are functional-correctness proofs to C code"* (<https://sel4.systems/About/FAQ.html>). All seL4 proofs are **uniprocessor** — *"multicore SMP … x64 VT-x virtualisation extensions"* are named as *"mature unverified configurations"* and multicore verification is still *"in progress"* (<https://sel4.systems/Verification/proofs.html>, FAQ). **Implication for JARVIS:** the current uniprocessor x64 build (Q2) runs a *genuinely verified* (C-level functional correctness) seL4 configuration — so the formal-verification guarantee really IS in force today, and enabling SMP (Branch A) would forfeit it. This *strengthens*, not weakens, the verification objection to Branch A. (more `formally verified` mentions to reconcile project-wide — follow-up.)
- **SMP forfeits the formal-verification guarantee** (the multicore/BKL kernel is not yet verified — *"verification in progress for static multikernel configs"*, FAQ) **and the guarantee is in force today** because the current uniprocessor x64 build is itself a verified configuration (C-level functional correctness — see RESOLVED finding above). Branch A must weigh this; record an ADR.
