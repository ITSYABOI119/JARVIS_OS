...
## Next — goal #2b N-c-2/N-c-3 (console) + goal #2 backlog → goals #3–7

1. **goal #2b N-c-2** — a ~60-line Python UDP receiver that decodes the `telemetry_packet_t` (`struct.unpack_from('<IBBHIII', …)`, magic + `zlib.crc32(pkt[:196])` validate) and prints/serves live box state.
2. **goal #2b N-c-3** — the honesty-corrected React console (UI seed = git-ignored `phase4/docs/ui_mockups/design-system/`) rendering the stream; correct all aspirational fields (no GPU / live-tok-s / SHIELD-block / "verified"). **This completes goal #2b "Done when" (console shows live, honest box state over the network).**
