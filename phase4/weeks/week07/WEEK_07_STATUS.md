...
## Next — goal #2b N-c-3 (console) + goal #2 backlog → goals #3–7

1. **goal #2b N-c-3** — the honesty-corrected React console (UI seed = git-ignored `phase4/docs/ui_mockups/design-system/`) consuming the receiver's `--json` stream; correct all aspirational fields (no GPU / live-tok-s / SHIELD-block / "verified"). **This completes goal #2b "Done when" (console shows live, honest box state over the network).**
