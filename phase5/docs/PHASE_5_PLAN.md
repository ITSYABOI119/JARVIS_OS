...
**Status:** **SUBSTANTIALLY COMPLETE (2026-07-04)** — all 7 goals have a real, proven mechanism and the §3 done-when was met by the MVP arc + #5. **Arc 1 ("it-remembers", #1/#2/#3/#6) COMPLETE + DEPLOYED (2026-07-03):** G1 episodic store ✅ + G2 shared context pool ✅ (both M0–M4 box-verified) + G3 retrieval ✅ (M0–M6; **default-ON since 2026-07-02**, box-verified live boot_id=11) + G6 cache growth ✅ (M0–M4; **default-ON since 2026-07-03**, box-verified live boot_id=12). **Arc 2 COMPLETE (mechanism-proven, gated-off in deploy, 2026-07-04):** #5 SHIELD failure-learning ✅ (M0–M4; monitor-only, criterion-2 risk-rise probe-proven, telemetry v5; gated-off — activates Phase 6) + #4 semantic memory ✅ (M0/M1/M2/M4; deterministic distill → separate fact store, write-only, telemetry v6; gated-off — the honest deployed yield is ~1 fact because #6 serves the recurring inference, `PHASE_5_GOAL4_SEMANTIC_MEMORY.md` §8) + **#7 consolidation FOLDED into #4** (the boot-scan `sd_distill` IS the compact-core; periodic cadence + prune deferred to Phase 6 / when there's signal). The deployed image intentionally diverges from v1.0.0 (the memory stack is ON). The `memory` milestone tag is PROPOSED as **`v1.1.0-memory`** (the user names/creates tags).
...
**Storage decision (grounded in the read):** carve a **raw-LBA MEMORY region near the start of the gap** — proposed **base ≈ sector 21,100,000, reserve ~8 GiB** (≈ 16,777,216 sectors → ends ≈ 37,877,215). This leaves comfortable margin after `JARVIS_DATA` (p2 ends at 21,094,399) and sits **~3.56 billion sectors away** from Ubuntu (p3) and the tail telemetry log — zero collision risk. Per-store sub-regions are carved **within** this 8 GiB and defined in the per-goal docs. **Sub-region map (RESERVED — an installer/repartition must never overlap these):** episodic **JEPI @ 21,100,000** (+8193 sectors → ends 21,108,192; `PHASE_5_GOAL1_EPISODIC_STORE.md`) · semantic **JSEM @ 21,110,000** (+4097 → ends 21,114,096; `PHASE_5_GOAL4_SEMANTIC_MEMORY.md`) · action-audit **JACT @ 21,120,000** (+4097 → ends 21,124,096; Phase 6 K — `phase6/docs/PHASE_6_GOAL_K_IT_ACTS.md` §5, landed at K/M0 as `action_audit.c/h`).
