...
**Status:** IN PROGRESS (started 2026-06-26). **The "it-remembers" MVP arc (#1/#2/#3/#6) is COMPLETE (2026-07-03):** G1 episodic store ✅ + G2 shared context pool ✅ (both M0–M4 box-verified) + G3 retrieval ✅ (M0–M6; **default-ON since 2026-07-02**, box-verified live boot_id=11) + G6 cache growth ✅ (M0–M4; **default-ON since 2026-07-03**, box-verified live boot_id=12 — `cache_growth_count=9`, promoted patterns serving at <1 ms, err=0). The deployed image intentionally diverges from v1.0.0 (the memory stack is ON). The early `memory` milestone tag is PROPOSED (`phase5/docs/GOAL6_FINISH_REPORT.md` §6 — the user names/creates tags). Next: Arc 2 (#4 semantic memory, #5, #7).
