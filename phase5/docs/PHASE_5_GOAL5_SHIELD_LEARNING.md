...
**Status:** 🚧 IN PROGRESS — **M0 ✅ host/CI (2026-07-03: `shield_learn.c/h` pure risk-map, Phase-1 parity, host-tested 28/28)** · **M1 ✅ LANDED (2026-07-03: gated `JARVIS_SHIELD_LEARN` PA wiring — boot-scan seed + [STATS]-cadence fold + `[SHIELD-LEARN]` summary + the gated `JARVIS_SHIELD_PROBE` D-d probe; box smoke = the gate)** · **M2 ✅ DONE (2026-07-04: the D-e telemetry/console slice — telemetry v5 222 B/CRC@218 `shield_learn_keys`/`shield_learn_max_risk_x100` + `TLM_F_SHIELD_LEARN` 0x200, receiver/fixture/golden lockstep, console monitor-only "Failure-learning" section + Capabilities row + honesty/e2e teeth; host green 47/106/53/14/24)** · **M3 satisfied-by-evidence** (the D-d probe's risk-RISES gate ran in the M1/M2 box smokes); M4 (docs + flag decision) → **DECIDED: #5 stays GATED-OFF in deploy** (err=0 workload → a live monitor honestly reads 0 forever; capability proven via the transient probe smoke, activation is Phase 6). **Arc 2 opener.**
**Date:** 2026-07-03
...
- **M2 (CI + BOX)** ✅ **DONE 2026-07-04** — the deliberate telemetry/console slice (D-e), fixture-synced like v2/v3/v4: telemetry **v5** (218→222 B, CRC@218, version 5) appends `shield_learn_keys` (keys with learned risk) + `shield_learn_max_risk_x100` (max learned adjustment ×100) + `TLM_F_SHIELD_LEARN` 0x200; the `main_x86.c` fill is gated `#if JARVIS_SHIELD_LEARN` (same table walk as the `[SHIELD-LEARN]` summary; flag set only when keys>0) so the **flag-OFF deploy emits 0s + flag clear** — honest; receiver decode + `packet_to_record` + FLAG_NAMES, fixture `_DEFAULTS`/`FLAG_BITS`, golden json (infer frame carries keys=1/max=20 — the D-d probe shape) + `golden.pcap` regenerated, `gen_golden_pcap` guards 222; console = the SHIELD screen's flag-gated **monitor-only "Failure-learning" section** ("learns which actions fail and raises their risk — monitor-only, not a live blocker; enforcement is Phase 6"; `—` until the flag is live; NEVER a blocked-count) + the "SHIELD failure-learning (monitor-only)" Capabilities row; honesty gate adds "SHIELD blocks"/"blocking active" to BANNED + asserts the monitor-only wording; e2e value-pins rendered == live `shield_learn_keys`. Host green: telemetry C 47/47, receiver 106/106, honesty 53/53, logic 14/14, e2e 24/24. Box smoke (transient `JARVIS_SHIELD_LEARN=1` + `JARVIS_SHIELD_PROBE=1`, QEMU/KVM): the v5 build compiles + probe drives the learned risk UP across attempts, err=0 — then flags restored to 0 (NOT deployed-ON).
- **M3 (BOX)** — **satisfied by evidence:** the D-d synthetic-failure probe ran in the M1 and M2 box smokes — the same failing action's learned risk RISES on the repeat attempt (+0.1/attempt, monotonic; on a store already carrying prior probe failures the rise continues from the seeded level, capped at +0.50) (**criterion 2 closed, honestly scoped**).
- **M4** — docs + week status ✅ (this doc + CLAUDE.md + week02, 2026-07-04); flag default decision **RESOLVED: stays default-0 (gated-off in deploy)** — unlike G3/G6 there is nothing for a default-ON monitor to show (the benign deployed workload runs err=0, so the counters would honestly read 0 forever and the console row `—`/0 is the correct display); the learning capability is proven (M0 host parity + M1/M2 probe smokes) and **activates in Phase 6** with enforcement + real failure sources.
