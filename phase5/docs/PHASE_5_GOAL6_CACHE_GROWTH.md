...
**Status:** ✅ **COMPLETE (M0–M4) — `JARVIS_CACHE_GROWTH` default-ON since #6/M3 (2026-07-03); DEPLOYED + box-verified live (boot_id=12).** M0 ✅ host/CI (selector + SEC-024 LRU live-tested) · M1 ✅ box (promotion pass, Option-B freq aggregate + HWM cap; `grow=6→9`) · M2 ✅ CI + box-confirm live (`cache_growth_count` telemetry + console row; `TLM_F_CACHE_GROWTH` + count=9 live over the real I211) · M3 ✅ box (M3a serve path → `served=42,404`, `infer` frozen at 17 while q hit 283,400 err=0; M3b coherent served heads; M3c `used` max 261 < HWM 409 — the LRU stays host-proven + unreachable-by-design, the SYSTEM_DESIGN §7 refinement of this doc's original "LRU fires on the box" wording; flip bar 6/6 → default-ON `99419fb`) · M4 ✅ docs + the `memory` tag PROPOSED (user creates tags). **Evidence + deviations: `phase5/docs/GOAL6_FINISH_REPORT.md`; at-scale forensics (boot_id=12 ~874K q err=0, reproduced boot_id=13 on the v4 build at 490 q/s / 84.9% hit-rate / err=0): `phase5/docs/GOAL6_SOAK_boot12_REPORT.md`. The it-remembers MVP arc (#1/#2/#3/#6) is CLOSED.** *(D-a: a first-cut live-routing "#6a" landed under `JARVIS_CACHE_GROWTH` and was **REVERTED** (`2650253`); this doc's promote-from-log is the canonical #6 — the M3a serve path is READ-only and never inserts on the inference route.)*
**Date:** 2026-06-30 (completed 2026-07-03)
