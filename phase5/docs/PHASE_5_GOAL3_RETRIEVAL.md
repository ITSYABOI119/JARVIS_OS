...
**Status:** ✅ **M6 SHIPPED 2026-07-02 — `JARVIS_G3_RETRIEVAL` default-ON; retrieval is DEPLOYED** — M0 ✅ (scorer + assembler, host/CI; now **53 PASS** incl. T8 exact-only / T9 anti-echo / T10 clean-truncation) · M1 ✅ **BOX** (PA packs the preamble; OFF↔ON byte-identical) · M2 ✅ **BOX** (PB injects between the user `\n` and the question, `prompt_ids[256]`, OFF byte-identical) · M3 ✅ **BOX 2026-06-28** (coherent prose via the `g3_candidate_usable` inference-record filter, `e4e4a56`) · M4 ✅ **BOX 2026-06-30** (<50 ms + v3 telemetry + console row + synthetic-fact probe `[PROBE] hit=1`) · M5 ✅ **BOX 2026-06-30** (NVMe-backed post-reboot recall, `[RECALL] index n=330`, 6× `recall=1`) · **M6 ✅ SHIPPED (see §6 M6 record: 3× offline A/B, P6+P7 fixed, blind panel net-neutral-to-slightly-positive, zero harmful failure modes).** **M4d ✅ BOX-VERIFIED LIVE 2026-07-03 (boot_id=11)** — **GOAL COMPLETE (M0–M6), retrieval deployed.**
**Date:** 2026-06-28 (M6 shipped: 2026-07-02; M4d box-verified: 2026-07-03)
...
- **M6** ✅ **SHIPPED 2026-07-02 — `JARVIS_G3_RETRIEVAL` flipped `0`→`1` (default-ON; retrieval deployed).** The flip-to-production-default was a **product call gated on the offline OFF-vs-ON A/B**, and that gate was cleared by **three A/Bs on the box** (`JARVIS_G3_AB` harness, deterministic `rng_state=42` query set, paired OFF/ON KVM runs):
  - **A/B1** (2026-07-01, on `9ba7007`; OFF=22/ON=10): net-neutral-to-slightly-positive but exposed **P6 cross-topic contamination** (recency-fallback + embedded prior-question text echoed a topically-distant fact into an seL4 answer) → **fixed `70ca236`** (`g3_select_exact_only` — recency fallback DROPPED for injection — + `g3_build_preamble_answer_only` fenced answer-only preamble).
  - **A/B2** (on `70ca236`): blind panel **net-POSITIVE** (ON 4/7, OFF 2, tie 1); P6 gone, but exposed **P7 self-echo** (dangling `### W` markdown fragment from mid-token truncation + verbatim lead-sentence restatement) → **fixed `2bb537b`** (`g3_clean_answer_len` clean-boundary truncation + "use as reference; add new detail, do not repeat" build-on label; `test_g3_retrieval.c` → 53 PASS).
  - **A/B3** (2026-07-02, on `2bb537b`; OFF=22/ON=13, `[RETR] hit=1` ×14): **ALL CLEAN** — `### W` = 0, zero contamination, zero label/instruction leakage, all responses coherent, `err=0`. Blind panel (13 pairs × 3 lenses + artifact-hunter + repeat-progression analysts): **net-neutral-to-slightly-positive** — ON clearly better on 2 question families (build-on compresses covered framing and reaches substantive list items OFF never reaches), one hair of lost precision on 1 family (TCP "sequence numbers"→"connection parameters"), rest ties; **zero materially-worse cases** (the greedy OFF baseline gives a repeat-asker byte-identical output — zero new information — so ON ≥ OFF everywhere).
  - **Key OFF-baseline finding:** the residual "lead-sentence restatement" on repeats (q=69/q=87) is the **model's natural greedy answer** — the OFF run produces the IDENTICAL opening sentence with no injection — so it is not an echo defect; the first-sentence-only excerpt follow-up was judged NOT worth its enrichment cost and was not taken.
  - Honesty stance unchanged: the shipped metrics are retrieval **hits + latency** (v3 telemetry, console rows); **"memory helped" is NOT a system claim** — that stays an offline-A/B question.
  - **M4d ✅ BOX-VERIFIED LIVE 2026-07-03 (boot_id=11, ESP-deployed default-ON build):** live I211 v3 telemetry — 34 packets `version=3`/216 B crc_ok, `flags_list` carrying `RETRIEVAL` (0x80), `retrieval_hits` climbing 1→2→3, `retrieval_latency_us` 192→5078 µs, `q_errors=0`, coherent Gemma, `episodic_count=584`; the console "Retrieval before inference" Capabilities row renders live. **G3 COMPLETE (M0–M6).**
