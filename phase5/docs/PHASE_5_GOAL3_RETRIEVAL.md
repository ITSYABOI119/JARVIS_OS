...
**Status:** 🚧 M0–M5 box-verified; M6 (flip default-ON) PENDING offline A/B — M0 ✅ (scorer + assembler, host/CI **30 PASS**) · M1 ✅ **BOX** (PA packs the preamble; OFF↔ON byte-identical) · M2 ✅ **BOX** (PB injects between the user `\n` and the question, `prompt_ids[256]`, OFF byte-identical) · M3 ✅ **BOX 2026-06-28** (coherent prose via the `g3_candidate_usable` inference-record filter, `e4e4a56`) · M4 ✅ **BOX 2026-06-30** (<50 ms + v3 telemetry + console row + synthetic-fact probe `[PROBE] hit=1`) · M5 ✅ **BOX 2026-06-30** (NVMe-backed post-reboot recall, `[RECALL] index n=330`, 6× `recall=1`) · **M6 🔜 PENDING offline A/B — `JARVIS_G3_RETRIEVAL` was flipped to 1 (`d4c58ff`) then PARKED back to 0 (opt-in): retrieval works + is used (`[PROBE] hit=1`), but M2/M3 showed it broadens answers, so whether it HELPS is the honest gate. Re-flip only after the A/B.**
...
- **M6** 🔜 PENDING the offline A/B — the flip-to-production-default is a **product call, not a code milestone**. `JARVIS_G3_RETRIEVAL` was flipped `0`→`1` (`d4c58ff`) then **PARKED back to 0** (opt-in default): retrieval demonstrably **works and is USED** (`[PROBE] hit=1` — the model echoes an injected, otherwise-unknowable fact), the wiring is sound (PA pack + PB inject both reach it; PA links `g3_retrieval.c`, PB uses the header's `static inline g3_prompt_budget`), and the console honesty gate is 40/40. **But whether it HELPS is unproven** — M2/M3 showed injection broadens/meta-izes answers, so the honest gate is an **offline OFF-vs-ON A/B** (Claude-judged helpfulness, never a CI gate per D-f). Re-flip to the shipped default only after the A/B shows it helps, not merely works.
