...
**Status:** ✅ COMPLETE (M0–M6) — M0 ✅ (scorer + assembler, host/CI **30 PASS**) · M1 ✅ **BOX** (PA packs the preamble; OFF↔ON byte-identical) · M2 ✅ **BOX** (PB injects between the user `\n` and the question, `prompt_ids[256]`, OFF byte-identical) · M3 ✅ **BOX 2026-06-28** (coherent prose via the `g3_candidate_usable` inference-record filter, `e4e4a56`) · M4 ✅ **BOX 2026-06-30** (<50 ms + v3 telemetry + console row + synthetic-fact probe `[PROBE] hit=1`) · M5 ✅ **BOX 2026-06-30** (NVMe-backed post-reboot recall, `[RECALL] index n=330`, 6× `recall=1`) · **M6 ✅ 2026-07-01 — `JARVIS_G3_RETRIEVAL` flipped default-ON (retrieval now runs in the deployed build); console honesty gate 40/40; `phase5/weeks/week01` status written. The default-ON build is behaviorally identical to the M2–M5 box builds (all ran with the flag ON) — one confirming boot recommended.**
**Date:** 2026-06-28 (M6: 2026-07-01)
...
- **M6** ✅ DONE 2026-07-01 — flipped `JARVIS_G3_RETRIEVAL` `0`→`1` in `phase3/src/sel4/jarvis_debug.h` so retrieval runs in the deployed build (both the PA pack **and** PB inject sites reach it; PA links `g3_retrieval.c` via `build_jarvis_x86.sh`, PB uses only the header's `static inline g3_prompt_budget`). The flip reproduces the exact build that **M2–M5 already box-verified** — those smokes all ran with the flag ON — so ON-behavior (inject + coherence + <50 ms + post-reboot recall) is already proven; M6 only changes it from opt-in to the shipped default. Console honesty gate green (`test_console_honesty.py` **40/40**); the "Retrieval before inference" Capabilities row + `retrieval_hits`/`retrieval_latency_us` System stats now populate from the now-live `TLM_F_RETRIEVAL` flag. `phase5/weeks/week01/WEEK_01_STATUS.md` written (resumes the lapsed Phase 5 cadence). **Remaining box-confirm (recommended, not a re-proof):** one boot of the default-ON image to re-observe `[RETR]`/`[RETR-PB]` firing by default + coherent generation + err=0.
