...
#else
/* x86 platform: debug output via stdio */
#include <stdio.h>
static void debug_puts(const char *s) { fputs(s, stderr); }
static void debug_hex32(uint32_t val) __attribute__((unused));
static void debug_hex32(uint32_t val) { fprintf(stderr, "%08x", val); }
#endif
...
#else
    /* x86: no GENET hardware, ARP resolve not available */
    (void)timeout_ms;
    debug_puts("[NET] ARP resolve: not available on x86\n");
#endif
    return false;
}
