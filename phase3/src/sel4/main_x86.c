                           ...
                           uint16_t recall_cos2_x1000, uint8_t recall_emit_mask)
{
     ...
     * The predicate IS the pa_ctrl_gate acceptance test -- it CALLS g3_candidate_usable rather
     * than restating it, so "index and filter cannot disagree" is true by construction and not by
     * comment. It used to be a hand-written restatement that omitted `action` entirely, which was
     * harmless only while every record in this store was tag 3.
...
#if JARVIS_EMBED_PROBE == 4
    /* READ-BACK: the record as the STORE hands it back, not as we built it. This closes the
     * writer -> NVMe callbacks -> store -> reader path, which is the only part a host test
     * cannot cover; and it prints the record's own seq, which the gate needs to tie a later
     * turn's src2 to the turn that produced it. static: PA keeps 512 B buffers off the frame
     * (the `rec` precedent directly above). */
    {
        static epi_record_t rb;
        if (cnt > 0 && epi_store_read(&g_ctrl_episodic, cnt - 1u, &rb) == 0) {
            puts_serial("[PROV] readback seq="); put_dec(rb.seq);
            puts_serial(" kind=");        put_dec((uint32_t)rb.recall_kind);
            puts_serial(" src=");         put_dec(rb.recall_src_seq);
            puts_serial(" cos_x1000=");   put_dec((uint32_t)rb.recall_cos_x1000);
...
#endif
    /* ACTION IS PART OF THE PREDICATE, and it has to be. A locally-served answer
     * (EPI_ACT_CONTROL_IN_LOCAL -- SYSFACTS or the canned DECLINE) is rendered by PA,
     * not generated by the model, so it is not a memory. The FETCH filter excludes it
     * for free (g3_candidate_usable keys on == EPI_ACT_CONTROL_IN) but THIS predicate
     * did not look at action at all, because every record here used to be tag 3.
     * Indexing a tag-4 record would recreate exactly the shadowing described above:
...
static void pa_ctrl_gate(const control_result_t *cres)
{
    ...
    uint32_t crk_src  = 0;
    uint16_t crk_cos  = 0;
    /* WIDENING (2026-09-03): the SELECTED-SET half of the provenance. Same lifetime and the
     * same by-construction reset as the triple above -- a turn that could not have recalled
     * records n = 0 because these are re-initialised per turn, not because anyone remembered. */
    uint8_t  crk_n    = 0;
    uint32_t crk_src2 = 0;
    uint16_t crk_cos2 = 0;
        ...
        if (cplen == 0 && g_embed_sem_ok && g_embed_idx_n > 0) {
            ...
            if (erc == 0 && prc == EMBED_PROJ_OK) {
                ...
                if (ss > 0) {                 /* the semantic pick REPLACED ctrl_sel[0] */
                    ...
                    /* The count is set ONLY where the kind is set. A turn whose selected records
                     * produced no preamble (cplen == 0) did not recall, and records n = 0 --
             ...
             * wrong direction, and the selector deliberately reports only what cleared the floor. */
            {
                ...
                /* The SECOND pick's score, captured in the SAME pass -- zero extra multiply-adds,
                 * since the loop already visits every candidate. Matched on (seq, query_key), the
                 * same pair g3_select_semantic's own "already taken" test uses; seq alone would be
                 * enough here but the pair is what the selector treats as identity. */
                double score2 = 0.0;
                for (int ci = 0; ci < sn; ci++) {
                    ...
                    if (ss >= 2 && g_sem_cands[ci].seq == ctrl_sel[1].seq
                                && g_sem_cands[ci].query_key == ctrl_sel[1].query_key)
                        score2 = d;
                }
                ...
                /* BYTE-FOR-BYTE the same truncation and clamp as crk_cos above, deliberately: the
                 * stored value must equal what soak_prov_driver.c prints as cos_x1000 for rank 2,
                 * or a reconstruction and a record would disagree by a rounding rule. */
                if (csem && ss >= 2) {
                    int b2 = (int)(score2 * 1000.0);
                    crk_cos2 = (uint16_t)(b2 < 0 ? 0 : (b2 > 65535 ? 65535 : b2));
                }
    ...
    if (!served_locally) {
        ...
        puts_serial("\"\n");
#if JARVIS_CONTROL_IN_RECALL
        /* THE TAG. A SYSFACTS or DECLINE answer was rendered by PA from live state or a
         * canned string -- it is not a memory, so it is written (audit + corpus preserved)
