 ...
 * types renamed to uint32_t/uint8_t/uint64_t via <stdint.h>. Public domain.
 *
 * Byte-exact copy of the reference round function and K-table — do not
 * hand-tune. No malloc, no libc beyond <string.h> (memcpy).
 */
#include "sha256.h"
#include <string.h>
...
void sha256_final(sha256_ctx *ctx, uint8_t out[32])
{
    ...
    /* Reverse byte ordering to produce the big-endian digest output. */
    for (i = 0; i < 4; ++i) {
