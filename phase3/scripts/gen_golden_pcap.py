...
Guards before writing: struct.calcsize(fmt)==236 (both FMT and the fixture's
...
def main():
    ...
    assert struct.calcsize(FMT) == 236, "FMT no longer 236 bytes (v8)"
    assert struct.calcsize(meta['fmt']) == 236, "meta.fmt no longer 236 bytes (v8)"
    assert meta['size'] == 236, "meta.size != 236"
