...
Guards before writing: struct.calcsize(fmt)==216 (both FMT and the fixture's
...
def main():
    ...
    assert struct.calcsize(FMT) == 216, "FMT no longer 216 bytes (v3)"
    assert struct.calcsize(meta['fmt']) == 216, "meta.fmt no longer 216 bytes (v3)"
    assert meta['size'] == 216, "meta.size != 216"
