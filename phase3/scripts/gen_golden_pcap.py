...
receiver's CURRENT-wire format (PKT_SIZE_V14 — derived, so no size literal here can
...
from telemetry_fixture import (  # noqa: E402
    FMT_V14, PKT_SIZE_V14, build_pcap_many, frame_to_packet)
...
def main():
    ...
    # size is DERIVED from PKT_SIZE_V14, so a future append needs no edit here and no literal in
    # this file can go stale (it used to hardcode 262 in three places).
    assert struct.calcsize(FMT_V14) == PKT_SIZE_V14, "FMT_V14/PKT_SIZE_V14 disagree"
    assert struct.calcsize(meta['fmt']) == PKT_SIZE_V14, (
        "meta.fmt packs to %d, current wire is %d — regenerate the fixture meta"
        % (struct.calcsize(meta['fmt']), PKT_SIZE_V14))
    assert meta['size'] == PKT_SIZE_V14, (
        "meta.size %r != current wire %d" % (meta['size'], PKT_SIZE_V14))
